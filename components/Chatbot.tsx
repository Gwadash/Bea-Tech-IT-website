...
const Chatbot: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && !chat) {
            try {
                ...
                setMessages([{ role: 'model', text: 'Hello! How can I help you today? Feel free to ask any questions about Bea-Tech IT or book an appointment.' }]);
            } catch (error) {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed bottom-24 right-6 w-full max-w-sm h-[70vh] max-h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col transition-all duration-300 origin-bottom-right transform scale-100 opacity-100">
                    <header className="bg-blue-600 text-white p-4 rounded-t-2xl flex justify-between items-center shadow-md">
